...
class GPTConfig:
    block_size : int = 1024
    vocab_size : int = 50257  # 50k merges, 256 bytes tokens and 1 <|endoftext|> token
    n_layer : int = 12
    n_head : int = 12
    n_embd : int = 768
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            wpe = nn.Embedding(config.block_size, config.n_embd),  # FIX: was vocab_size
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        decay_params = [p for n,p in param_dict.items() if p.ndim >= 2] #decay weights and 2d+ tensors
        nodecay_params = [p for n,p in param_dict.items() if p.ndim < 2] #dont decay biases and 1d tensors
        optim_groups = [
